...
1. **Upload & model selection**
   * Choose one of the curated free OpenRouter models (DeepSeek, Mistral, Ollama, etc.).
   * Upload `.pdf`, `.docx`, or `.txt`. The backend stores the file in a session-scoped temp directory.
   * Optional “Test LLM” button exercises the OpenRouter pipeline using the legacy concatenated prompt format.
...
3. **Determine headers**
   * Feeds the document to OpenRouter in ≤120k-token slices.
...
5. **Async passes & merge**
   * For each pass, chunks are ranked via entropy + domain keywords and truncated to a 120k-token budget before dispatching asynchronous OpenRouter calls.
...
* Update `backend/app.py` → `OPENROUTER_FREE_MODELS` to adjust the model shortlist.
