...
def detect_headers(chunks, embeddings, clusters, profile, context: RAGContext):
    ...
    normalize_labels = [normalize_text(label) for label in seg_cfg.get("normalize_within", [])]
    ...
    for idx, chunk in enumerate(chunks):
        ...
        if token_boosts:
            token_hits = sum(1 for token in token_boosts if token in text_lower)
        ...
        normalize_boost = 0.0
        if normalize_labels:
            hits = sum(1 for label in normalize_labels if label and label in text_lower)
            if hits:
                normalize_boost = 0.05 * hits
